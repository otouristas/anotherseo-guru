      ...
      <Card className="p-4 sm:p-6 bg-gradient-to-br from-primary/5 to-secondary/5 border-primary/20">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          ...
          <div className="text-left lg:text-right w-full lg:w-auto">
            ...
            <div className="text-3xl sm:text-4xl font-bold text-primary">82</div>
            <Progress value={82} className="w-full lg:w-32 mt-2" />
          </div>
        </div>
      </Card>
          ...
          return (
            <Card key={idx} className="p-3 sm:p-4 md:p-6 hover:shadow-lg transition-shadow">
              <div className="flex items-start justify-between mb-2 sm:mb-4">
                ...
                <Badge variant={metric.trendUp ? "default" : "secondary"} className="gap-1 text-xs">
                  {metric.trendUp ? <TrendingUp className="w-2 h-2 sm:w-3 sm:h-3" /> : <TrendingDown className="w-2 h-2 sm:w-3 sm:h-3" />}
                  {metric.trend}
                </Badge>
              </div>
              <div className="text-xl sm:text-2xl md:text-3xl font-bold mb-1">{metric.value.toLocaleString()}</div>
      ...
      <div className="grid md:grid-cols-2 gap-4 sm:gap-6">
        <Card className="p-4 sm:p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-base sm:text-lg font-semibold">Recent SERP Changes</h3>
          </div>
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="flex items-center justify-between py-2 border-b last:border-0">
                <div className="flex-1 min-w-0 pr-2">
                  <div className="font-medium text-xs sm:text-sm truncate">Keyword #{i}</div>
                  <div className="text-xs text-muted-foreground truncate">example.com/page</div>
                </div>
        ...
        <Card className="p-4 sm:p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-base sm:text-lg font-semibold">Content Calendar</h3>
          </div>
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs sm:text-sm text-muted-foreground">Scheduled</span>
              <Badge className="text-xs">{stats.content}</Badge>
            </div>
            <div className="flex items-center justify-between">
            ...
            <div className="flex items-center justify-between">
              <span className="text-xs sm:text-sm text-muted-foreground">Published This Month</span>
